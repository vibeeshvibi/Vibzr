    ...
    <AnimatePresence>
      {isFullScreenOpen && (
        ...
        >
          ...
          <div className="relative z-10 flex flex-col h-full px-6 pb-8 pt-[calc(env(safe-area-inset-top)+1rem)] max-w-lg mx-auto w-full overflow-y-auto">
            {/* Top Bar */}
            ...
            <AnimatePresence>
              {showPlaylistMenu && (
                <motion.div
                  ...
                  exit={{ opacity: 0, y: -10 }}
                  className="absolute top-20 right-6 bg-brand-surface border border-white/10 rounded-xl p-2 z-20 min-w-48"
                >
              ...
              <div className="relative">
                <img
                  ...
                  className={`rounded-2xl object-cover shadow-2xl transition-all duration-300 ${
                    showQueue ? 'w-44 h-44 sm:w-56 sm:h-56' : 'w-64 h-64 sm:w-76 sm:h-76'
                  }`}
                  onError={(e) => {
            ...
            <div className="flex items-start justify-between mb-4 flex-shrink-0">
              <div className="flex-1 min-w-0 mr-4">
                ...
                {currentTrack.album && (
                  <p className="text-white/40 text-xs mt-0.5 truncate">{currentTrack.album}</p>
                )}
              </div>
            ...
            <button
              ...
              className={`flex items-center gap-2 mx-auto text-sm font-medium transition-colors mb-2 flex-shrink-0 ${showQueue ? 'text-brand-pink' : 'text-white/40 hover:text-white'}`}
            >
              <ListMusic size={16} />
            ...
            <AnimatePresence>
              {showQueue && (
                ...
                >
                  <div className="max-h-60 overflow-y-auto space-y-1.5 pr-1 touch-pan-y">
                    {queue.map((track, i) => (
                      <div
                        key={`${track.id}-${i}`}
                        onClick={() => usePlayerStore.getState().setTrack(track, queue)}
                        className={`flex items-center gap-3 p-2 rounded-xl cursor-pointer transition-all
                                    ${track.id === currentTrack.id ? 'bg-brand-pink/20 border border-brand-pink/30' : 'hover:bg-white/5 bg-white/[0.03]'}`}
                      >
